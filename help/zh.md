...
> **注意**：中文音频不支持强制对齐和音高提取，中文音频只能在文本标注模式下进行标注。
...
- **重新标注**：与语料库管理同一套服务；MIPVU 使用 Clause 小句模型进行全标注，仅支持英语；NRC 支持中英文。
...
**摘要标注流程**：每条带摘要的文献会写入影子语料库并执行与「语料库管理」相同的标注流水线：SpaCy → USAS → MIPVU → NRC。MIPVU 使用 Clause 小句模型（deberta-v3-large-clause-metaphor）进行全词性隐喻标注；NRC 情感标注基于 NRC-EmoLex 词典，支持中英文，为情感分析模块提供数据基础。
...
在「可视化」标签页中，可以生成多种可视化图表。系统提供 **9 个可视化标签页**，标签页过多时会自动出现左右滚动按钮：
...
6. **引文弦图（Citation Chord）** — 施引/被引期刊弦图
7. **山脊图（Ridgeline Plot）** — 景观图风格的密度山脊
8. **热力密度图（Heatmap）** — 等值线密度 + 散点叠加
9. **词云（Word Cloud）** — 标题/摘要词云
...
大多数图表（网络图、聚类图、时间线、时区视图、引文弦图、山脊图）共享同一配色方案选择器，提供 7 种选项：蓝色（Blue）、绿色（Green）、紫色（Purple）、橙色（Orange）、红色（Red）、青色（Teal）、多彩（Colorful）。突增检测和热力密度图使用各自独立的配色，词云使用专属色阶。
...
| 参数 | 范围 | 默认值 | 说明 |
|------|------|--------|------|
| 横轴缩放 | 0.5–5 | 1 | 拉伸或压缩横轴总长度，可通过水平滚动条查看全貌 |
...
- **节点圆圈**：大小反映权重（被引次数 + 术语数 + 微小偏移保证唯一性）；权重精度越高，圆圈大小差异越显著；同年份节点按权重从大到小堆叠，有一定水平偏移避免完全重叠
- **连线弧线**：二次贝塞尔曲线弧线连接相关节点。**泳道内连线**（同聚类内共享关键词的节点）和**跨泳道连线**（不同聚类间共享 2 个及以上关键词的节点）均会显示，跨泳道弧线弯曲度随 Y 轴距离自动调整
...
引文弦图（Citation Chord）以双弧弦图形式展示施引期刊和被引期刊之间的引用关系。左侧弧为施引期刊，右侧弧为被引期刊，贝塞尔曲线连接表示引用流。

#### 配置参数

| 参数 | 范围 | 默认值 | 说明 |
|------|------|--------|------|
| 弧线角度 | 30°–90° | 90° | 控制左右弧线的张开角度；角度越小弧线越平，连线越短 |
| 配色方案 | 7 种 | 蓝色 | 施引/被引节点和连线的配色 |

#### 图表特点

- **双弧布局**：左弧 = 施引期刊（Citing），右弧 = 被引期刊（Cited）
- **弧上节点**：大小反映期刊的引用/被引频次
- **连线流光**：贝塞尔曲线连接施引与被引，线宽反映引用量，悬停高亮关联节点
- **弧线角度自适应**：弧线角度变化时保持图表可视高度不变（通过 `R = H / sin(θ)` 计算半径）
- **标签自适应**：弧度变小后节点跨度不足时自动隐藏标签，同时进行碰撞检测避免重叠

---

### 山脊图

山脊图（Ridgeline Plot）以景观图（Landscape）风格展示聚类的密度分布。每个聚类占一行，横轴为年份，使用面积曲线表示各年份的文献密度。

#### 配置参数

| 参数 | 范围 | 默认值 | 说明 |
|------|------|--------|------|
| 横轴缩放 | 0.5–5 | 1 | 拉伸或压缩横轴总长度 |
| 配色方案 | 7 种 | 多彩 | 每个聚类使用不同颜色渐变；「多彩」模式使用彩虹渐变 |

#### 图表特点

- **动态年份间距**：与时间线视图相同，每年的横轴空间根据文献量动态分配
- **山脊叠加**：每个聚类的面积曲线略有上移，产生山脊叠加效果
- **渐变填充**：面积曲线使用从浅到深的颜色渐变
- **水平滚动**：横轴缩放 > 1 时出现水平滚动条
- **自定义 X 轴**：因动态间距采用手绘刻度线 + 年份标签

---

### 热力密度图

热力密度图使用 `d3.contourDensity()` 计算 2D 密度等值线，以连续渐变色展示文献在坐标空间中的聚集程度。

#### 配置参数

| 参数 | 范围 | 默认值 | 说明 |
|------|------|--------|------|
| 带宽 | 0.05–2.0 | 0.15 | 核密度估计的带宽参数，值越大越平滑 |
| 色阶 | Turbo / Blue / Green / Purple / Orange / Red / Teal | Turbo | 热力图色阶，不同于通用配色方案 |

#### 图表特点

- **等值线层**：多层等值线填充，颜色从浅到深表示密度从低到高
- **散点叠加**：原始数据点以半透明圆圈叠加在等值线之上
...
- **格式**：SVG（矢量图）
- **优点**：可缩放，适合打印和编辑
- **全图导出**：对于有水平滚动条的图表（时间线、山脊图等），导出时会自动捕获完整的 SVG 内容（使用 `getBBox()` 计算），不受视口滚动限制
...
5. 使用**突增检测**定位研究热点时段
6. 使用**引文弦图**分析期刊引用流
7. 使用**山脊图**观察密度分布和聚类形态
8. 导出图表用于论文或报告

### 参数调优建议

- **网络图节点过多**时，提高「最小权重」或降低「最大节点数」
- **时间线太挤或太空**时，调节「横轴缩放」（> 1 拉伸，< 1 压缩）
- **时间线节点大小差异不明显**时，增加「权重精度」（保留更多小数）
- **引文弦图标签过于密集**时，减小「弧线角度」
- **热力图等值线太粗糙**时，减小「带宽」值；太平滑时增大「带宽」
- **聚类图凸包太多**时，增大「凸包阈值」
...
- 时间线和山脊图的动态年份间距会自动根据文献量调整，横轴刻度间距不等
...
> **重要**：音频波形标注功能仅支持英语音频。中文音频只能在文本标注模式下进行标注。
...
- 系统使用 `saves/nrc/` 目录下与语料语言对应的词典文件（如英文：English-NRC-EmoLex.txt，中文：Chinese-Simplified-NRC-EmoLex.txt）。
...
选择 USAS 语义域标注使用的方式：**规则模式**、**神经网络模式**或**混合模式**。界面会显示各模式在当前环境下是否可用（依赖后端依赖与模型）。

### 规则模式 (Rule-based)

- 基于词典与规则标注，支持多词表达式（MWE），结合话语域与一文一义消歧。
- **特点**：速度快、可解释性强；适合一般场景。

### 神经网络模式 (Neural)

- 基于预训练语义标注模型，能利用上下文。
- **特点**：上下文理解好；适合需要更高语义准确度的场景。
...
- **状态**：页面会显示英文/中文语义资源是否可用。
- **预设文本类型**：系统预设类型（如 GEN 一般文本）以折叠列表展示，可点击「编辑」修改该类型的优先语义域。
- **自定义文本类型**：若有自定义类型，在另一折叠列表中展示；同样可编辑优先语义域，自定义类型可删除。
- 编辑时在弹窗中选择语义域（可多选），保存后生效。
