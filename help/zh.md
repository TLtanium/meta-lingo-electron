...
#### 第 4 步：小句 Clause 模型二次检测（混合）

针对 HiTZ 模型在功能词上的局限性，使用基于 SpaCy 小句训练的 DeBERTa 小句模型，对 IN（介词）、DT（限定词）、RB（副词）、RP（小品词）进行二次检测：
...
基于 VUA 语料库测试集（10 个文件，23,588 词，仅统计字母词）的评估结果：

#### 整体性能对比

| 方案 | F1 | 精确率 | 召回率 | 准确率 |
|------|-----|--------|--------|--------|
| HiTZ 单模型 | 60.6% | 89.90% | 45.67% | 93.56% |
| Clause 单模型 | 75.2% | 80.04% | 70.86% | 94.93% |
| **混合方案** | **84.8%** | 89.59% | **80.53%** | **96.88%** |
| **相对 HiTZ 提升** | **+24.2%** | -0.31% | **+34.86%** | +3.32% |

#### 按词性分析

| 评估范围 | 精确率 | 召回率 | F1 |
|----------|--------|--------|-----|
| HiTZ 在 IN/DT/RB/RP（四类功能词） | 100.00% | 4.25% | **8.2%** |
| Clause 在 IN/DT/RB/RP | 89.55% | 81.93% | **85.6%** |
| **混合方案在 IN/DT/RB/RP** | 89.67% | 83.08% | **86.3%** |
| HiTZ 非功能词（其他词性） | 89.51% | 78.51% | **83.7%** |
| 混合方案非功能词 | 89.51% | 78.51% | **83.7%** |

#### 关键发现

1. **HiTZ 模型局限**：在四类功能词（IN/DT/RB/RP）上 F1 仅约 **8.2%**，召回率极低（4.25%），几乎不报隐喻；在非功能词上表现优异（F1≈83.7%）。
2. **Clause 小句模型**：专门覆盖 IN/DT/RB/RP，四类功能词 F1 达 **85.6%**，整体单模型 F1 **75.2%**。
3. **混合方案优势**：非功能词沿用 HiTZ，功能词在 HiTZ 判非隐喻时交由 Clause 再判；整体 F1 从 HiTZ 的 60.6% 提升至 **84.8%**，四类功能词 F1 提升至 **86.3%**，非功能词保持 83.7%。
4. **跨文本类型**：在 10 篇测试文本（新闻/学术/小说/口语）中，混合方案整体 F1 几乎均显著优于 HiTZ 与单独 Clause。
5. **推理效率**：M3 Pro (MPS) 上约 **7.22 秒/千词**，适合离线标注与分析。
...
- **重新标注**：头部按钮组提供 SpaCy、USAS、MIPVU 重新标注。与语料库管理使用同一套服务：MIPVU 为 HiTZ + Clause 混合流程，仅支持英语。
...
**摘要标注流程**：每条带摘要的文献会写入影子语料库并执行与「语料库管理」相同的标注流水线：SpaCy → USAS → MIPVU。MIPVU 使用与语料库管理一致的混合流程（HiTZ 模型 + Clause 小句模型），输出包含整体及按词性分组的统计（IN/DT/RB/RP/其他）。
