...
| **MI³** | -∞ to +∞ | 进一步减少低频偏差 |
| **Dice** | 0-1 | 条件概率的调和平均值 |
| **Delta P1** | -1 to 1 | 方向性关联：节点词→搭配词 |
| **Delta P2** | -1 to 1 | 方向性关联：搭配词→节点词 |
| **MinSens** | 0-1 | 保守的双向关联度量 |

默认启用4种度量：LogDice、MI、Delta P1 和 Delta P2。用户可通过统计方法对话框启用/禁用度量、设置阈值和调整列顺序。
...
- **饼图**：排名靠前的搭配词的比例分布
- **搭配网络图**：D3.js 力导向图，节点词居中，搭配词环绕，大小由关联分数决定
...
底部面板的「文本标注列表」和「视频标注列表」标签页以表格形式展示所有标注，并支持以下操作：

#### 操作菜单

每行标注的操作列提供下拉菜单（点击 ⋮ 按钮展开），包含以下选项：

- **搭配分析**：跳转至共现关系/搭配分析模块，以该标注词语为搜索词（需已选择语料库）
- **词图分析**：跳转至词图分析模块，以该标注词语为搜索词（需已选择语料库）
- **N-gram 分析**：跳转至 N-gram 分析模块，以该标注词语为搜索词（需已选择语料库）
- **删除标注**：删除该条标注记录

> **提示**：跨域链接功能需要当前多模态标注会话已关联语料库。若未选择语料库，以上分析选项将显示为禁用状态。

#### 行点击定位

点击标注表格中的任意一行，可将该标注在转录文本中对应的位置高亮并自动滚动到可视区域：

- 选中行显示彩色轮廓，与标注颜色一致
- 对应的转录句子在「转录文本标注」标签页中自动定位高亮
- 再次点击同一行可取消选中
