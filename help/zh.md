...
Meta-Lingo 采用三步检测方案，结合规则过滤与 Clause 小句模型全标注：
...
#### 第 3 步：Clause 小句模型全标注

对通过前两步过滤的**所有**词语，以完整句子为上下文传入 Clause 小句模型进行隐喻判断：

- **输入**：完整句子（提供小句级上下文）
- **输出**：每个词的二分类预测（non_metaphor / metaphor）
- **判断阈值**：P(LABEL_1) >= 0.5
- **最大序列长度**：192 tokens（与模型训练配置一致）
- **来源标签**（用于界面颜色区分）：
  - 功能词 IN/DT/RB/RP → **橙色**标签（间接隐喻）
  - 非功能词（其他词性）→ **绿色**标签（间接隐喻）
