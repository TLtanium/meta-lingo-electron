...
同义词分析模块（也称词族分析）基于 NLTK WordNet 词典，自动识别语料库中词语的同义词关系。该模块支持**语料库/文献库**统一选择（与词频统计等模块一致），结果仅保留**在所选语料中实际出现过的同义词**（与词典同义关系取交集），便于发现词汇替换模式与语义关联；支持网络图与树状图可视化，以及跨模块链接（共现关系、搭配分析、词图分析、N-gram、语义域分析等）。
...
- **词频统计**、**共现关系**、**搭配分析**、**词图分析**、**N-gram 分析**、**语义域分析**等模块，并以当前词语为搜索词
...
- **查看共现关系**：打开共现关系页签，并以**语义域**为条件进行 CQL 查询（匹配方式为「包含」，仅忽略 _MWE 后缀），查看该语义域下所有词汇的共现关系。
...
词频统计、同义词分析、关键词提取（单文档与关键性对比）、共现关系、搭配分析（搭配分析/词图分析/词图对比）、主题建模（BERTopic/LDA/LSA/NMF）以及标注模式的词操作菜单中，均提供**语义域分析**链接选项，配置同上。
...
- **跨模块链接**：每行有到共现关系模块的链接，自动同步搭配跨距为上下文大小并高亮搭配词
...
- **功能**：
  ...
  - **操作列（跨域链接）**：每行提供 ⋮ 菜单，包含四项跨域链接：
    - **查看共现关系**：打开共现关系（KWIC/CQL）页，以该标注词语为搜索词
...
- **常规模式**：主题关键词的操作菜单提供「搭配分析」「共现关系」「词图分析」「N-gram 分析」链接。
- **N-gram 模式**：仅提供「共现关系」链接。
...
语义域模式下，每行对应一个语义域，跨模块链接**仅提供共现关系**（与关键词提取的语义域对比模式一致）：

- **共现关系**：以 `[usas="域代码"]` 形式的 CQL 查询打开搭配/共现检索，显示该语义域下所有词语的语境，便于深入考察语义域内词语的实际用法。
...
| **共现关系** | 数据源、词性、检索方式与检索词、排序；KWIC 结果表当前页或密度图/山脊图的数据。 |
