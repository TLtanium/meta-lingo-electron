...
文献可视化模块用于管理和分析学术文献数据，支持从 Web of Science (WOS) 和中国知网 (CNKI) 导入 Refworks 格式的文献数据。支持为条目上传 PDF、生成首页缩略图，以及基于 PDF/摘要的 11 项 AI 生成内容（研究目标、研究设计等）；提供相关度星级（含零星、点击当前星取消）、标签与备注编辑，以及导出 CSV（不含论文列）和导出详情 PDF。可视化分析包括合作网络、关键词共现、时区视图和突增检测等。
