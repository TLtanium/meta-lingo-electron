...
### v3.8.98 (2026-01-29)
- **修复**: 编码者间信度标准答案功能验证错误
  - **后端返回 summary**: `/api/reliability/calculate` 端点现在正确返回 `summary` 字段，前端可获取数据摘要
  - **字段访问逻辑修复**: `_annotation_to_key` 函数现在正确处理字段值为 `None` 的情况，避免 `dict.get()` 返回意外值
  - **标准答案索引计算修复**: 前端 `goldIndex` 现在基于实际成功加载的存档列表计算，避免部分存档加载失败导致索引错位
  - **翻译键补充**: 添加缺失的 `reliability.archives` 翻译键
