...
import type { POSFilterConfig, POSTagInfo } from '../../../types/collocation'

interface CollocationPOSFilterProps {
...
export default function CollocationPOSFilter({
  config,
  onChange,
  posTags,
  disabled = false
}: CollocationPOSFilterProps) {
    ...
    >
      ...
      <AccordionDetails>
            ...
            <FormControlLabel
              ...
              label={
                <Typography variant="body2">
                  {t('collocation.posFilter.keepMode')}
                </Typography>
              }
            />
            <FormControlLabel
              ...
              label={
                <Typography variant="body2">
                  {t('collocation.posFilter.filterMode')}
                </Typography>
              }
            />
          </RadioGroup>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
          {config.keepMode
            ? (isZh ? '仅保留选中的词性' : 'Keep only selected POS tags')
            : (isZh ? '过滤掉选中的词性' : 'Filter out selected POS tags')
          }
        </Typography>

        {/* Info about all parts matching */}
        {config.selectedPOS.length > 0 && (
          <Alert 
            severity="info" 
            icon={false}
          ...
          >
            <Typography variant="caption">
              {t('collocation.posFilter.allPartsMatch')}
            </Typography>
          </Alert>
        )}
      </AccordionDetails>
