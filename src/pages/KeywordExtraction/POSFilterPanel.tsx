/**
 * POS Filter Panel Component for Keyword Extraction
...
import type { POSFilterConfig, POSTagInfo } from '../../types/keyword'
...
export default function POSFilterPanel({
  ...
  // Show warning when keep mode is selected but no POS tags are selected
  const showWarning = config.keepMode && config.selectedPOS.length === 0
  ...
  return (
    ...
    >
      <AccordionSummary expandIcon={<ExpandMoreIcon />}>
        <Stack direction="row" alignItems="center" spacing={1}>
          ...
          <Typography variant="subtitle2">
            {t('keyword.posFilter.title', 'POS Filter')}
          </Typography>
      ...
      <AccordionDetails>
            ...
            <FormControlLabel
              ...
              label={
                <Typography variant="body2">
                  {t('keyword.posFilter.keepMode', 'Keep')}
                </Typography>
              }
            />
            <FormControlLabel
              ...
              label={
                <Typography variant="body2">
                  {t('keyword.posFilter.filterMode', 'Filter')}
                </Typography>
              }
            />
          </RadioGroup>
        ...
        {/* Warning for keep mode with no selection */}
        {showWarning && (
          <Alert severity="warning" sx={{ mb: 2, py: 0 }}>
            <Typography variant="caption">
              {t('keyword.posFilter.keepModeWarning', 'Please select at least one POS tag in keep mode')}
            </Typography>
          </Alert>
        )}
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
          {config.keepMode
            ? t('keyword.posFilter.keepModeDesc', 'Only keep words with selected POS tags')
            : t('keyword.posFilter.filterModeDesc', 'Filter out words with selected POS tags')
          }
        </Typography>
      </AccordionDetails>
    </Accordion>
