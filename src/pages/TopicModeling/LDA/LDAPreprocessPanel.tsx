...
import type { LDAPreprocessConfig, POSTagInfo } from '../../../types/topicModeling'
import { topicModelingApi } from '../../../api'

interface LDAPreprocessPanelProps {
  corpusId: string
  textIds: string[]
  language: string
  config: LDAPreprocessConfig
  onConfigChange: (config: LDAPreprocessConfig) => void
}
...
export default function LDAPreprocessPanel({
  corpusId,
  textIds,
  language,
  config,
  onConfigChange
}: LDAPreprocessPanelProps) {
  ...
  const handleConfigChange = (key: keyof LDAPreprocessConfig, value: unknown) => {
  ...
  const handlePreview = async () => {
    if (!corpusId || textIds.length === 0) {
      setPreviewError(t('topicModeling.lda.selectTextsFirst', 'Please select texts first'))
      return
    }
    ...
    try {
      const response = await topicModelingApi.previewLDAPreprocess(
        corpusId,
        textIds,
        language,
        config,
  ...
  return (
    <Paper sx={{ p: 2, mb: 2 }}>
      <Typography variant="subtitle1" fontWeight={600} gutterBottom>
        {t('topicModeling.lda.preprocess.title', 'Preprocessing')}
      </Typography>
      ...
      <Stack spacing={2}>
        ...
        <Box sx={{ p: 1.5, bgcolor: 'action.hover', borderRadius: 1 }}>
          <Typography variant="body2" color="text.secondary">
            {t('topicModeling.lda.preprocess.languageInfo', 'Language')}: {' '}
            <Chip
            ...
            {language === 'chinese' && (
              <Typography variant="caption" color="text.secondary" sx={{ ml: 1 }}>
                ({t('topicModeling.lda.preprocess.jiebaTokenize', 'jieba tokenization')})
              </Typography>
            )}
          </Typography>
        </Box>
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lda.preprocess.ngramMode', 'N-gram mode')}
              </Typography>
            }
          />
          {config.ngram_enabled && (
            <Box sx={{ mt: 1, pl: 3 }}>
              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                {t('topicModeling.lda.preprocess.selectNValues', 'Select N value(s) (multi-select)')}
              </Typography>
              <Stack direction="row" flexWrap="wrap" gap={0.5}>
                {([2, 3, 4, 5, 6] as const).map((n) => {
                  const label = n === 2
                    ? t('topicModeling.lda.preprocess.bigram', 'Bigram(2)')
                    : n === 3
                      ? t('topicModeling.lda.preprocess.trigram', '3-gram')
                      : t('topicModeling.lda.preprocess.nGram' + n, `${n}-gram`)
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0.5 }}>
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lda.preprocess.removeStopwords', 'Remove stopwords')}
              </Typography>
            }
          />
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lda.preprocess.lemmatize', 'Lemmatize')}
              </Typography>
            }
          />
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lda.preprocess.lowercase', 'Lowercase')}
              </Typography>
            }
          />
        </Box>
        ...
        {/* Min word length */}
        <NumberInput
          label={t('topicModeling.lda.preprocess.minWordLength', 'Min word length')}
          size="small"
        ...
        {/* Exclusion Words */}
        <TextField
          label={t('topicModeling.lda.preprocess.exclusionWords', 'Exclusion Words')}
          multiline
          rows={3}
          size="small"
          fullWidth
          ...
          helperText={t('topicModeling.lda.preprocess.exclusionWordsHelp', 'Custom words/patterns to exclude. Supports regular expressions.')}
        />
        ...
        <Box>
          ...
          <Stack direction="row" spacing={2}>
            <NumberInput
              label={t('topicModeling.lda.preprocess.minDf', 'Min DF')}
              size="small"
              value={config.min_df}
              ...
              integer
              sx={{ flex: 1 }}
              helperText={t('topicModeling.lda.preprocess.minDfHelp', 'Min docs containing word')}
            />
            <NumberInput
              label={t('topicModeling.lda.preprocess.maxDf', 'Max DF')}
              size="small"
              value={config.max_df}
              ...
              step={0.05}
              sx={{ flex: 1 }}
              helperText={t('topicModeling.lda.preprocess.maxDfHelp', 'Max ratio of docs')}
            />
          </Stack>
        </Box>
                ...
                >
                  <Stack direction="row" spacing={1} alignItems="center" mb={0.5}>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      {preview.original_token_count} -&gt; {preview.processed_token_count} tokens
                    </Typography>
        ...
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography variant="h6">
              {t('topicModeling.lda.preprocess.detail', 'Preprocessing Detail')}
            </Typography>
            {selectedPreview && (
              <>
                <Chip
                  label={`${selectedPreview.original_token_count} tokens`}
                  size="small"
                  variant="outlined"
                />
                <Chip
                  label={`-> ${selectedPreview.processed_token_count} tokens`}
                  size="small"
                  color="primary"
                />
              </>
            ...
            <Stack spacing={2}>
              <Box>
                <Typography variant="subtitle2" gutterBottom>
                  {t('topicModeling.lda.preprocess.original', 'Original')}:
                </Typography>
                <Box sx={{
                  bgcolor: 'action.hover',
                  p: 2,
              ...
              <Box>
                <Typography variant="subtitle2" gutterBottom>
                  {t('topicModeling.lda.preprocess.processed', 'Processed')}:
                </Typography>
                <Box sx={{
