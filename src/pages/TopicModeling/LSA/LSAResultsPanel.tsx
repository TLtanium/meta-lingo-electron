/**
 * LSA Results Panel
 * Display LSA analysis results with topic cards and table (same format as BERTopic)
 */
...
export default function LSAResultsPanel({ 
  ...
  const handleSaveLabel = useCallback(async () => {
    ...
    setSavingLabel(true)
    try {
      const response = await topicModelingApi.updateLSATopicLabel(
        result.result_id,
        editingTopicId,
        editingLabel
      )
      ...
      console.error('Failed to save LSA label:', err)
    } finally {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    csvRows.push([
      t('topicModeling.lsa.viz.topicId', 'Topic ID'),
      t('topicModeling.results.topicName', 'Topic Name'),
      t('topicModeling.lsa.viz.topKeywords', `Top ${keywordDisplayCount} Keywords (Weight)`),
      t('topicModeling.lsa.viz.numDocuments', 'Number of Documents'),
      t('topicModeling.lsa.viz.docPercentage', 'Percentage of Documents (%)'),
      t('topicModeling.lsa.viz.explainedVariance', 'Explained Variance (%)'),
      t('topicModeling.lsa.viz.representativeKeywords', `Representative Keywords (Top ${topN})`)
    ].join(','))
    ...
    result.topics.forEach((topic, idx) => {
      ...
      const variance = result.individual_variance?.[idx] 
        ? (result.individual_variance[idx] * 100).toFixed(2) 
        : 'N/A'
    ...
    csvRows.push([
      ...
      result.explained_variance_ratio != null 
        ? `Total Explained Variance: ${(result.explained_variance_ratio * 100).toFixed(2)}%` 
        : '',
      result.singular_values_sum != null 
        ? `Singular Values Sum: ${result.singular_values_sum.toFixed(2)}` 
        : ''
    ].join(','))
    ...
    link.href = url
    link.download = `lsa_topics_${new Date().toISOString().split('T')[0]}.csv`
    link.click()
    
    URL.revokeObjectURL(url)
  }, [result, keywordDisplayCount])
  ...
  const selectedTopicIndex = result.topics?.findIndex(t => t.topic_id === selectedTopicId) ?? -1
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            <Chip label={`${result.vocabulary_size} ${t('topicModeling.lsa.results.vocabulary', 'Vocabulary')}`} variant="outlined" />
          </Stack>
          <Stack direction="row" spacing={2} alignItems="center">
            {result.explained_variance_ratio != null && (
              <Chip 
                label={`${t('topicModeling.lsa.results.explainedVariance', 'Explained Variance')}: ${(result.explained_variance_ratio * 100).toFixed(2)}%`}
                variant="outlined"
                color="success"
              />
            )}
            ...
            <Tooltip title={t('topicModeling.lsa.results.exportTopics', 'Export topics table (CSV)')}>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={tabValue} onChange={(_, v) => setTabValue(v)}>
          ...
          <Tab label={t('topicModeling.lsa.results.docTopics', 'Document-Topic Distribution')} />
        </Tabs>
      </Box>
                ...
                <Box sx={{ 
                  ...
                  <Stack direction="row" alignItems="center" spacing={1}>
                    <Chip 
                      ...
                      variant="outlined"
                    />
                    {selectedTopicIndex >= 0 && result.individual_variance?.[selectedTopicIndex] !== undefined && (
                      <Chip 
                        label={`${(result.individual_variance[selectedTopicIndex] * 100).toFixed(1)}% var`}
                        size="small"
                        variant="outlined"
                        color="success"
                      />
              ...
              <Table size="small" stickyHeader>
                <TableHead>
                  <TableRow sx={{ '& th': { bgcolor: theme.palette.mode === 'dark' ? '#1e1e2e' : 'grey.100', fontWeight: 600 } }}>
                    ...
                    <TableCell align="right" sx={{ width: 100 }}>
                      {t('topicModeling.lsa.results.explainedVariance', 'Variance')}
                    </TableCell>
                ...
                <TableBody>
                  {result.topics?.map((topic, index) => {
                    const docCount = topicCounts[topic.topic_id] || 0
                    const variance = result.individual_variance?.[index]
          ...
          <Paper sx={{ borderRadius: 2, overflow: 'hidden' }}>
              ...
              <Table size="small" stickyHeader>
                <TableHead>
                  <TableRow sx={{ '& th': { bgcolor: 'grey.100', fontWeight: 600 } }}>
                    <TableCell>{t('topicModeling.lsa.results.docId', 'Doc')}</TableCell>
                    <TableCell>{t('topicModeling.lsa.results.dominantTopic', 'Dominant Topic')}</TableCell>
                    <TableCell>{t('topicModeling.lsa.results.weight', 'Weight')}</TableCell>
                    <TableCell>{t('topicModeling.lsa.results.distribution', 'Distribution')}</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
            ...
            {result.doc_topics.length > 100 && (
              <Box sx={{ p: 1, borderTop: 1, borderColor: 'divider' }}>
                <Typography variant="caption" color="text.secondary">
                  {t('topicModeling.lsa.results.showingFirst', 'Showing first 100 of')} {result.doc_topics.length} {t('common.items')}
                </Typography>
              </Box>
            )}
          </Paper>
        ...
        {tabValue === 2 && (!result.doc_topics || result.doc_topics.length === 0) && (
          <Box sx={{ 
            ...
            <Typography color="text.secondary">
              {t('topicModeling.lsa.results.noDocTopics', 'No document-topic distribution data')}
            </Typography>
          </Box>
        )}
      </Box>
