...
import type { LSAPreprocessConfig, POSTagInfo } from '../../../types/topicModeling'
import { topicModelingApi } from '../../../api'

interface LSAPreprocessPanelProps {
  corpusId: string
  textIds: string[]
  language: string
  config: LSAPreprocessConfig
  onConfigChange: (config: LSAPreprocessConfig) => void
}
...
export default function LSAPreprocessPanel({
  corpusId,
  textIds,
  language,
  config,
  onConfigChange
}: LSAPreprocessPanelProps) {
  ...
  // Load POS tags on mount (using LDA service)
  useEffect(() => {
    loadPOSTags()
  }, [])
  ...
  const handleConfigChange = (key: keyof LSAPreprocessConfig, value: unknown) => {
  ...
  // Handle preview (uses LSA preprocess endpoint)
  const handlePreview = async () => {
    if (!corpusId || textIds.length === 0) {
      setPreviewError(t('topicModeling.lsa.selectTextsFirst', 'Please select texts first'))
      return
    }
    ...
    try {
      const response = await topicModelingApi.previewLSAPreprocess(
        corpusId,
        textIds,
        language,
        config,
  ...
  return (
    <Paper sx={{ p: 2, mb: 2 }}>
      <Typography variant="subtitle1" fontWeight={600} gutterBottom>
        {t('topicModeling.lsa.preprocess.title', 'Preprocessing')}
      </Typography>
      ...
      <Stack spacing={2}>
        ...
        <Box sx={{ p: 1.5, bgcolor: 'action.hover', borderRadius: 1 }}>
          <Typography variant="body2" color="text.secondary">
            {t('topicModeling.lsa.preprocess.languageInfo', 'Language')}: {' '}
            <Chip
            ...
            {language === 'chinese' && (
              <Typography variant="caption" color="text.secondary" sx={{ ml: 1 }}>
                ({t('topicModeling.lsa.preprocess.jiebaTokenize', 'jieba tokenization')})
              </Typography>
            )}
          </Typography>
        </Box>
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lsa.preprocess.ngramMode', 'N-gram mode')}
              </Typography>
            }
          />
          {config.ngram_enabled && (
            <Box sx={{ mt: 1, pl: 3 }}>
              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                {t('topicModeling.lsa.preprocess.selectNValues', 'Select N value(s) (multi-select)')}
              </Typography>
              <Stack direction="row" flexWrap="wrap" gap={0.5}>
                {([2, 3, 4, 5, 6] as const).map((n) => {
                  const label = n === 2
                    ? t('topicModeling.lsa.preprocess.bigram', 'Bigram(2)')
                    : n === 3
                      ? t('topicModeling.lsa.preprocess.trigram', '3-gram')
                      : t('topicModeling.lsa.preprocess.nGram' + n, `${n}-gram`)
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0.5 }}>
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lsa.preprocess.removeStopwords', 'Remove stopwords')}
              </Typography>
            }
          />
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lsa.preprocess.lemmatize', 'Lemmatize')}
              </Typography>
            }
          />
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                {t('topicModeling.lsa.preprocess.lowercase', 'Lowercase')}
              </Typography>
            }
          />
        </Box>
        ...
        {/* Min word length */}
        <NumberInput
          label={t('topicModeling.lsa.preprocess.minWordLength', 'Min word length')}
          size="small"
        ...
        <Box>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
            {t('topicModeling.lsa.preprocess.dfFilter', 'Document Frequency Filter')}
          </Typography>
          <Stack direction="row" spacing={2}>
            <NumberInput
              label={t('topicModeling.lsa.preprocess.minDf', 'Min DF')}
              size="small"
              value={config.min_df}
              ...
              integer
              sx={{ flex: 1 }}
              helperText={t('topicModeling.lsa.preprocess.minDfHelp', 'Min docs containing word')}
            />
            <NumberInput
              label={t('topicModeling.lsa.preprocess.maxDf', 'Max DF')}
              size="small"
              value={config.max_df}
              ...
              step={0.05}
              sx={{ flex: 1 }}
              helperText={t('topicModeling.lsa.preprocess.maxDfHelp', 'Max ratio of docs')}
            />
          </Stack>
        </Box>
        ...
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography variant="h6">
              {t('topicModeling.lsa.preprocess.detail', 'Preprocessing Detail')}
            </Typography>
            {selectedPreview && (
              <>
                <Chip
            ...
            <Stack spacing={2}>
              <Box>
                <Typography variant="subtitle2" gutterBottom>
                  {t('topicModeling.lsa.preprocess.original', 'Original')}:
                </Typography>
                <Box sx={{
                  bgcolor: 'action.hover',
                  p: 2,
              ...
              <Box>
                <Typography variant="subtitle2" gutterBottom>
                  {t('topicModeling.lsa.preprocess.processed', 'Processed')}:
                </Typography>
                <Box sx={{
