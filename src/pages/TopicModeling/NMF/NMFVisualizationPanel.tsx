/**
 * NMF Visualization Panel
 * D3.js visualizations for NMF results with interactive features
 */
...
import type { NMFResult, NMFDynamicResult, NMFTopic } from '../../../types/topicModeling'
...
interface NMFVisualizationPanelProps {
  result: NMFResult | NMFDynamicResult | null
}
...
export default function NMFVisualizationPanel({ result }: NMFVisualizationPanelProps) {
  ...
  const handleExportSVG = useCallback(() => {
    ...
    link.href = url
    link.download = `nmf-${vizType}-chart.svg`
    link.click()
  ...
  const handleExportPNG = useCallback(async () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        const link = document.createElement('a')
        link.href = url
        link.download = `nmf-${vizType}-chart.png`
        link.click()
  ...
  // Transform NMF topics to TopicWordBars format
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        ...
        >
          ...
          <Tab value="topicPie" label={t('topicModeling.nmf.viz.topicPie', 'Topic Distribution')} />
          <Tab value="docDist" label={t('topicModeling.nmf.viz.docDist', 'Document Distribution')} />
          {result?.has_dynamic && (
        ...
        {!hasData ? (
          <Box sx={{ 
            ...
            <Typography variant="h6" color="text.secondary">
              {t('topicModeling.visualization.noResult', 'No Results')}
            </Typography>
            <Typography variant="body2" color="text.secondary" textAlign="center">
              {t('topicModeling.visualization.runAnalysisFirst', 'Run NMF analysis to see visualizations')}
            </Typography>
          </Box>
        ) : (
          <Box sx={{ 
...
// Document Distribution Chart with interactivity
